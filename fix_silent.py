...
# Make sure we import sync if not already imported (wait, we can just use a normal map with a mutex, but wait, there is cartoonMutex or just a map. Let's define a map globally in commands.go)
...
if "var AutoJoinGroups bool" in content:
    content = content.replace("var AutoJoinGroups bool", inject_map)
elif "seenGroupLinks =" not in content:
    # Find where AutoJoinGroups is defined
...
if old_logic in content:
    ...
    with open('/home/lennox/Desktop/اهها/Go_Bot/internal/commands/commands.go', 'w') as f:
        f.write(content)
    print("Replaced HandleSyrian silently!")
else:
    print("Not found! Here's the current func:")
    print(content[content.find("func HandleSyrian(ctx *BotContext) {"):content.find("func HandleSyrian(ctx *BotContext) {")+1000])
